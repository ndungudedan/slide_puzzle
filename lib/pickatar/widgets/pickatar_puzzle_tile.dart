...
/// {@endtemplate}
class PickatarPuzzleTile extends StatefulWidget {
  /// {@macro dashatar_puzzle_tile}
  const PickatarPuzzleTile({
    Key? key,
    required this.tile,
    required this.state,
  ...
  @override
  State<PickatarPuzzleTile> createState() => PickatarPuzzleTileState();
}

/// The state of [PickatarPuzzleTile].
@visibleForTesting
class PickatarPuzzleTileState extends State<PickatarPuzzleTile>
  ...
  void initState() {
    ...
    // _animatedSwitch = switchNormalTile();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final hasStarted = status == PickatarPuzzleStatus.started;
    final puzzleLoading =   context.select(
              (PickatarPuzzleBloc bloc) => bloc.state.secondsToBegin,
            ) >=
            2;
    print(
        context.select((PickatarPuzzleBloc bloc) => bloc.state.secondsToBegin));
    print(puzzleLoading);
    final puzzleIncomplete =
        context.select((PuzzleBloc bloc) => bloc.state.puzzleStatus) ==
            PuzzleStatus.incomplete;

    final movementDuration = status == PickatarPuzzleStatus.loading
        ? const Duration(milliseconds: 800)
        : const Duration(milliseconds: 370);
    final birdEyeFound = context
        .select((PuzzleBloc bloc) => bloc.state.selectedCheekyBirds)
            ...
            child: ScaleTransition(
              ...
              child: IconButton(
                ...
                icon: AnimatedSwitcher(
                  ...
                  child: Image.asset(
                    birdEyeFound || 
                            (widget.tile.isCheekyBird &&!hasStarted && puzzleLoading) 
                        ? '${theme.dashAssetsDirectory}/eye.png'
  ...
  Widget switchNormalTile() {
    return Image.asset(
      context
          .select((PickatarThemeBloc bloc) => bloc.state.theme)
          .dashAssetForTile(widget.tile),
      semanticLabel: context.l10n.puzzleTileLabelText(
        widget.tile.value.toString(),
        widget.tile.currentPosition.x.toString(),
        widget.tile.currentPosition.y.toString(),
  ...
  Widget switchEyeTile() {
    return Image.asset(
      '${context.select((PickatarThemeBloc bloc) => bloc.state.theme).dashAssetsDirectory}eye.png',
      semanticLabel: context.l10n.puzzleTileLabelText(
        widget.tile.value.toString(),
        widget.tile.currentPosition.x.toString(),
        widget.tile.currentPosition.y.toString(),
